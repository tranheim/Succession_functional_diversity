...
Diversity of P-cycling genes (Kegg orthology)
...
names(t1) = "pcyc_diversity_fungi"
t1$Sample = row.names(t1)
names(t2) = "pcyc_diversity_bacteria"
...
```{r test for differences in fungal nog diversity}

#Check data distribution
hist(metadata$pcyc_diversity_fungi, xlab = "Response", main = "")
shapiro.test(metadata$pcyc_diversity_fungi) #Data is normally distributed

#Test for differences in grasslands relative to forest reference level
mod1 <- lmer(pcyc_diversity_fungi ~ Succession + (1|Par) + (1|Parent_material) + (1|Distance),
             data = metadata_ref)
...
mod2 = lmer(pcyc_diversity_fungi ~ Ecosystem_type + (1|Par), REML = T, data = metadata)
...
```{r bacterial nog diversity}

#Check data distribution
hist(metadata$pcyc_diversity_bacteria, xlab = "Response", main = "")
shapiro.test(metadata$pcyc_diversity_bacteria) #Data is normally distributed

#Run model
mod1 <- lmer(pcyc_diversity_bacteria ~ Succession +  (1|Par) + (1|Parent_material) + (1|Distance),
             data = metadata_ref)

#Check model disagnostics
resid <- simulateResiduals(fittedModel = mod1, plot = F)
plot(resid) #QQ-plot looks ok, some outliers but looks ok
...
#Estimate effect size using eta squared
mod2 = lmer(pcyc_diversity_bacteria ~ Ecosystem_type + (1|Par), REML = T, data = metadata)
...
t1 = metadata %>% dplyr::select(Sample, pcyc_diversity_fungi, pcyc_diversity_bacteria)
t2 = rbind(d1,d2)

write.table(t1, file = "./Results/pcyc_diversity.txt")
write.table(t2, file = "./Results/es_pcyc_diversity.txt")
